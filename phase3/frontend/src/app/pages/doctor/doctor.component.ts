...
export class DoctorComponent implements OnInit {
    ...
    if (this.date.trim().length == 0) {
      ...
      this.http
        .patch<addSlotModelResponse>(
          'https://backend-ali-elmallah-dev.apps.sandbox-m4.g2pi.p1.openshiftapps.com/addAppointment',
          body
        )
        .subscribe((data) => {
          if (data != null) {
  ...
  getDoctors(){
    this.http.post<GetDoctorsList[]>('https://backend-ali-elmallah-dev.apps.sandbox-m4.g2pi.p1.openshiftapps.com/getDoctorAppointments',{doctorID: this.doctorId}).subscribe((data) => {
      if (data != null) {
        this.slots = data
      }
    });
