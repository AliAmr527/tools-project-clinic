...
export class PatientComponent implements OnInit {
  ...
  CancelFunction(val:Number){
    this.http.post("http://localhost:5000/cancelReservation",{PatientID:this.patientID,slotID:val}).subscribe((data)=>{
      if(data!=null){
  ...
  getDoctors(){
    this.http.get<getDoctorsResponse[]>("http://localhost:5000/getDoctors",).subscribe((data)=>{
      if(data!=null){
  ...
  reserveFunction(){
    this.http.patch("http://localhost:5000/reserveSlot",{slotID:this.slotID,patientID:this.patientID}).subscribe((data)=>{
      if(data!=null){
  ...
  getSlots(){
    this.http.post<getSlotsModelResponseBody[]>("http://localhost:5000/getDoctorAppointmentsWithId",{doctorID:this.selectedDr}).subscribe((data)=>{
      if(data!=null){
  ...
  getReservedSlots(){
    this.http.post<getSlotsResponse[]>("http://localhost:5000/getReservations",{patientID:this.patientID}).subscribe((data)=>{
      if(data!=null){
        this.slots=data
        console.log(data)
      }
